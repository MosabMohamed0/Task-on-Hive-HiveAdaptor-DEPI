...
import 'package:depi/sessions_tasks/expence_tracker_hive/add_edit_screen.dart';
...
class _HomeScreenState extends State<HomeScreen> {
  ...
  Future<void> _loadExpenses() async {
    final data = await hiveDatabase.getAllExpense();
    setState(() {
      expenses = data;
    });
  }
  ...
  void _addOrEditExpense({ExpenseModel? expense, int? index}) async {
    ...
    if (result != null) {
      if (index == null) {
        await hiveDatabase.insertNewExpense(result);
      } else {
        await hiveDatabase.updateExpense(result);
      }
      _loadExpenses();
      setState(() {});
    }
  ...
  void _deleteExpense(int id) async {
    await hiveDatabase.deleteExpense(id);
    _loadExpenses();
    setState(() {});
  }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: Text("Expense Tracker (Hive)"),
        actions: [
          IconButton(
